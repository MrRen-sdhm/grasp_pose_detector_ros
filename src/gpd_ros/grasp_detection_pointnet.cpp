#include <gpd_ros/grasp_detection_pointnet.h>


/** constants for input point cloud types */
const int GraspDetectionPointnet::POINT_CLOUD_2 = 0; ///< sensor_msgs/PointCloud2
const int GraspDetectionPointnet::CLOUD_INDEXED = 1; ///< cloud with indices
const int GraspDetectionPointnet::CLOUD_SAMPLES = 2; ///< cloud with (x,y,z) samples


GraspDetectionPointnet::GraspDetectionPointnet(ros::NodeHandle& node) : has_cloud_(false), has_normals_(false),
                                                                        size_left_cloud_(0), has_samples_(true), frame_(""), use_importance_sampling_(false)
{
    ...
    if (cloud_type == POINT_CLOUD_2)
        ...
        cloud_sub_ = node.subscribe(cloud_topic, 1, &GraspDetectionPointnet::cloud_indexed_callback, this);
    else if (cloud_type == CLOUD_SAMPLES)
    {
        cloud_sub_ = node.subscribe(cloud_topic, 1, &GraspDetectionPointnet::cloud_samples_callback, this);
        has_samples_ = false;
    }
    ...
    {
        samples_sub_ = node.subscribe(samples_topic, 1, &GraspDetectionPointnet::samples_callback, this);
        has_samples_ = false;
    }
...
std::vector<int> GraspDetectionPointnet::getSamplesInBall(const PointCloudRGBA::Ptr& cloud,
...
void GraspDetectionPointnet::cloud_indexed_callback(const gpd_ros::CloudIndexed& msg)
{
    if (!has_cloud_)
    {
...
void GraspDetectionPointnet::cloud_samples_callback(const gpd_ros::CloudSamples& msg)
{
    if (!has_cloud_)
    {
        initCloudCamera(msg.cloud_sources);
...
void GraspDetectionPointnet::samples_callback(const gpd_ros::SamplesMsg& msg)
{
    if (!has_samples_)
    {
        Eigen::Matrix3Xd samples(3, msg.samples.size());
...
void GraspDetectionPointnet::initCloudCamera(const gpd_ros::CloudSources& msg)
{
    // clean up
    delete cloud_camera_;
    cloud_camera_ = NULL;
